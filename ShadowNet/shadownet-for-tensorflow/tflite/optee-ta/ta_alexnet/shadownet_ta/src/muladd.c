...
void neon_relu_32x4x4(float *x, float *y, float32x4x4_t const_0) {
    const float32x4x4_t  xx =
    {
        {
            vld1q_f32(x),
    ...
    float32x4x4_t yy = {
            {
                vmaxq_f32(const_0.val[0], xx.val[0]),
                vmaxq_f32(const_0.val[1], xx.val[1]),
                vmaxq_f32(const_0.val[2], xx.val[2]),
                vmaxq_f32(const_0.val[3], xx.val[3]),
            }
        };
...
int neon_relu(float *input, float *output, int size) {
    ...
    for (i = 0; i < size/16; i++) {
        neon_relu_32x4x4(input+i*16, output+i*16, const_0);
    }
