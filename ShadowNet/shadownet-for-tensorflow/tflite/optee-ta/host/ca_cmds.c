...
void network_predict_ca_cmd(SHADOWNET_CFG_IDX sn_idx, void *input, int in_len, void *output, int out_len)
{
  ...
  if (res != TEEC_SUCCESS) {
      ...
      printf("TEEC_InvokeCommand(RegisterSharedMemory) failed 0x%x, try load input", res);
  ...
  if (fetch_results_needed == 1) {
        ...
        // TODO: for evaluation purpose, comment out memory copy
        //printf("no memory copy from input to output\n");
    	//start = clock();
        //memcpy(output, input, out_len);
    	//end = clock();
    	//cpu_time_used = ((double) (end - start)) / CLOCKS_PER_SEC * 1000.0;
    	//printf("copy results time:%f ms\n", cpu_time_used);
  }
  printf("predict time: %d ms\n", op.params[2].value.a);
...
void setup_tee_session(void)
{
    ...
    LOGD("TEEC_InitializeContext entry");
