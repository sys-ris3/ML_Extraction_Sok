...
def convert_dwconv_layer(model, layer, obf_model, dw_layer):
    # copy dw weights
    ws = model.layers[layer].get_weights()
    obf_model.layers[dw_layer].set_weights(ws)

    # handle BatchNormalization
    bn_ws = model.layers[layer + 1].get_weights()
    obf_model.layers[dw_layer + 1].set_weights(bn_ws)
    return
...
def mobilenet_converter_custom(orig_model, obf_model_tpl, filled_model, obf_ratio):
    ...
    for idx in range(1,14):
        ...
        convert_dwconv_layer(model, dwlid, obf_model, obf_dwlid)
    ...
    # Conv Preds 
    layer_name = 'obf_conv_preds'
    layer_id = obfm_layermap[layer_name]
    convert_pred_layer(model, 90, obf_model, layer_id, obf_ratio)
...
if __name__ == '__main__':
    ...
    obf_custom_tpl = 'mobilenet_obf_onlypw.h5'
    filled_model= 'mobilenet_obf_onlypw_filled.h5'
    obf_ratio = 1.2
