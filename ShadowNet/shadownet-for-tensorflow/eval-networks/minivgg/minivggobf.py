...
def MiniVGGObf(input_shape, classes, obf_ratio, scalar_stack):
    ...
    # block 4 
    x = conv_obf(x, 128, obf_ratio, (3, 3), 'same', 4, 'relu', scalar_stack, True)
    x = BatchNormalization(axis = -1)(x) # Channel last

    # block 5 
    x = conv_obf(x, 128, obf_ratio, (3, 3), 'same', 5, 'relu', scalar_stack, True)
    x = BatchNormalization(axis = -1)(x) # Channel last
    x = MaxPooling2D(pool_size=(2,2), strides= (2,2))(x)
    x = Dropout(0.25)(x)
    ...
    x = conv_obf(x, 512, obf_ratio, (1,1), 'same', 6, 'relu', scalar_stack, True)
    x = BatchNormalization(axis = -1)(x) # Channel last
    x = Dropout(0.5)(x)

    # block 7 
    x = conv_obf(x, classes, obf_ratio, (1,1), 'same', 7, 'softmax', scalar_stack, True)
    x = Reshape((1000,))(x)

    model = training.Model(img_input, x, name='minivggobf')
    return model
