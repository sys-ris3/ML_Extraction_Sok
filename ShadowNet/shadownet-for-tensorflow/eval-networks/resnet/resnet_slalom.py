...
#from tensorflow.python.keras.engine.topology import load_weights_from_hdf5_group_by_name, h5py
...
def ResNet50(include_top=True, weights='imagenet', input_shape=None,
...
if __name__ == '__main__':
    model = ResNet50(include_top=True, weights="imagenet", input_shape=(224, 224, 3), pooling=None)
    #export_saved_model(model, 'resnet_slalom_wtf_11.h5')
    
    for i, w in enumerate(model.weights): 
        model.weights[i]._handle_name = model.weights[i].name + "_" + str(i)
        
    print(model.summary())
    model.save("resnet_slalom.h5")