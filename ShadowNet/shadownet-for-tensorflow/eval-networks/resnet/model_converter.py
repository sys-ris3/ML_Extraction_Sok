...
def convert_conv_layer(orig_model, conv_layer_num, obf_model, obf_conv_layer_num, obf_ratio, stacked = False):
    original_conv_layer = orig_model.layers[conv_layer_num]
    ...
    obf_conv_layer = obf_model.layers[obf_conv_layer_num]
    assert('conv2d' in obf_conv_layer.name)
    
    assert(original_conv_layer.name == obf_conv_layer.name)

    obf_conv_layer.set_weights([obf_weight])

    if not stacked:
        obf_linear_transform_layer = obf_model.layers[obf_conv_layer_num + 1]
    else:
        obf_linear_transform_layer = obf_model.layers[obf_conv_layer_num + 2]
    assert('linear_transform' in obf_linear_transform_layer.name)
    m_weight = obf_linear_transform_layer.get_weights()[0]
    s_weight = obf_linear_transform_layer.get_weights()[1]
    populate_linear_transform_weight(m_weight, s_weight, arr, obf_dict)
    obf_linear_transform_layer.set_weights([m_weight,s_weight,bias])
    return
