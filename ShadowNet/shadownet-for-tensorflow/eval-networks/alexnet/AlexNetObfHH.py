...
def AlexNetObf(input_shape, num_classes, obf_ratio, scalar_stack):
    ...
    x = conv_obf(img_input, 96, obf_ratio, (11, 11), 4, 'valid', 1, 'relu', scalar_stack, False)
    x = MaxPooling2D(pool_size=(3,3), strides= (2,2),
                          padding= 'valid', data_format= None)(x)

    x = conv_obf(x, 256, obf_ratio, (5,5), 1, 'same', 2, 'relu', scalar_stack, True)
    x = MaxPooling2D(pool_size=(3,3), strides= (2,2),
                          padding= 'valid', data_format= None)(x) 

    x = conv_obf(x, 384, obf_ratio, (3,3), 1, 'same', 3, 'relu', scalar_stack, True)
    x = conv_obf(x, 384, obf_ratio, (3,3), 1, 'same', 4, 'relu', scalar_stack, True) 
    ...
    x = conv(x, 4096, (1,1), 1,'same', 6, 'relu')
    x = conv(x, 4096, (1,1), 1,'same', 7, 'relu')
    x = conv(x, 1000, (1,1), 1,'same', 8, 'relu')
    x = conv(x, num_classes, (1,1),1,'same',9,'softmax')
    x = Reshape((1000,))(x)
