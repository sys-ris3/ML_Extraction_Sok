...
REGISTER_OP("TeeShadowGeneric_2Inputs")
    ...
    .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
      shape_inference::ShapeHandle input1_shape;
      shape_inference::ShapeHandle input2_shape;
    ...
    // assuming data format NHWC
      ...
      TF_RETURN_IF_ERROR(c->GetAttr("pos", &position));
      ...
      if(position.compare("results") == 0) { 
        // results shape(batch, channels)
        output_dims.emplace_back(c->Dim(input1_shape, 0));
        output_dims.emplace_back(chn_handle); 
        c->set_output(0, c->MakeShape(output_dims));
      } else { 
        TF_RETURN_IF_ERROR(c->ReplaceDim(input1_shape, channel_dim_index, chn_handle, &output_shape));
