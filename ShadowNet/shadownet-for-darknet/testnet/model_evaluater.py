...
"""
def evaluate_model_parts(model, obf_model, mp, omp, inputs):
    mo = run_submodel(mp, model, inputs)
    print("model output:")
    print(mo)
    omo = run_submodel(omp, obf_model, inputs)
    print("obf_model output:")
    print(omo)
    return compare_tensor(mo, omo)
...
"""
def evaluate_complex_model(orig_model, obf_enc_model):
    # load original model
    ...
    mp = range(0,4)
    omp = range(0,5)
    errs, err_rs, avg_a = evaluate_model_parts(model, oe_model, mp, omp, img_input)
    print("")
    ...
    omps = [(6,18),(19, 32), (33, 45), (46, 59), (60, 72), (73, 86), (87,99), (100,112),(113,125),(126,138), (139,151), (152, 165), (166,178),(181,186)]
...
"""
evaluate simple one layer model
"""
def evaluate_simple_model(orig_model_file, obf_model_file):
    model = tf.keras.models.load_model(orig_model_file)
    model.summary()
    
    obf_model = tf.keras.models.load_model(obf_model_file, custom_objects={'AddMask':AddMask,'LinearTransform':LinearTransform,'ShuffleChannel':ShuffleChannel})
    obf_model.summary()
    ...
    mp = range(0,2)
    omp = range(0,5)
    errs, err_rs, avg_a = evaluate_model_parts(model, obf_model, mp, omp, img_input)
...
if __name__ == '__main__':
    ...
    obf_model = 'testnet_obf_filled.h5'
    (la,lb,aa,ab,avga,acca) = evaluate_simple_model(orig_model, obf_model)
